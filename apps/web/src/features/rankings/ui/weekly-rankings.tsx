...
export const WeeklyRankings: React.FC<WeeklyRankingsProps> = ({ myNickname }) => {
  ...
  const { data: rankings, isLoading, isFetching } = useWeeklyRankings(currentPage);
  ...
  const formatDateRange = () => {
    if (!rankings.weekStart || !rankings.weekEnd) return "";
    const start = new Date(rankings.weekStart);
    const end = new Date(rankings.weekEnd);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center py-4 border border-border/20 rounded-lg bg-muted-foreground/5">
        <p className="text-sm font-light text-foreground">{formatDateRange()}</p>
        <p className="text-xs text-muted-foreground/60 font-light mt-1">이번 주 랭킹</p>
      </div>
